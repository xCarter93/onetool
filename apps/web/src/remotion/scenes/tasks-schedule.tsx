...
export const TasksSchedule: React.FC = () => {
	...
	const doneCount = TASKS.filter((task) => task.doneAt > 0 && frame >= task.doneAt).length;
	const ringTarget = doneCount / TASKS.length;
	const ring = interpolate(frame, [60, 160], [0, 1], {
	...
	return (
		<AbsoluteFill>
			<AppFrame
				active="Tasks"
				title="Today"
				headerRight={<StatusBadge status="in-progress" label="Tuesday, Aug 4" />}
			>
				<div style={{ position: "absolute", inset: 0, padding: 40, display: "flex", gap: 28 }}>
					...
					<Panel style={{ flex: 1.6, padding: 26, ...fadeUp(frame, 6, 16) }}>
						...
						{TASKS.map((task, i) => {
							const done = task.doneAt > 0 ? progress(frame, task.doneAt, 12) : 0;
							return (
								<div
									key={task.title}
									style={{
						...
						<Panel style={{ padding: 30, flex: 1, ...fadeUp(frame, 36, 16) }}>
							...
							{["Mon", "Tue", "Wed", "Thu", "Fri"].map((day, i) => {
								...
								return (
									<div
										key={day}
										style={{ display: "inline-flex", flexDirection: "column", alignItems: "center", width: "19%", verticalAlign: "bottom" }}
									>
