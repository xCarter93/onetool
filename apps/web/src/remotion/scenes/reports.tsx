	...
	return (
		<AbsoluteFill>
			<AppFrame
				active="Reports"
				title="Reports"
				headerRight={<StatusBadge status="viewed" label="Last 12 months" />}
			>
					...
					<div style={{ display: "flex", gap: 26, flex: 1 }}>
						...
						<Panel style={{ flex: 2.1, padding: 28, ...fadeUp(frame, 26, 16) }}>
							...
							<svg viewBox={`0 0 ${CHART_W} ${CHART_H}`} style={{ width: "100%" }}>
								<defs>
									<pattern id="ot-stripes" width={8} height={8} patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
								...
								<path d={area} fill="url(#ot-stripes)" opacity={reveal} />
								<path d={line} fill="none" stroke={t.chart1} strokeWidth={5} strokeLinecap="round" strokeLinejoin="round" />
								{/* Highlight last point */}
								<circle
									cx={pts[SERIES.length - 1][0]}
									cy={pts[SERIES.length - 1][1]}
									r={10 * pop(frame, fps, 100)}
									fill={t.chart1}
									stroke="white"
									strokeWidth={4}
								/>
							...
							<div
								style={{
									position: "absolute",
									right: 46,
									top: interpolate(reveal, [0, 1], [220, 96]),
									...fadeUp(frame, 104, 12, 10),
								}}
							>
