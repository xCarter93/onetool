...
import { HEADER_ACTION_RIGHT, headerCenterY, NAV_INDEX } from "../ui/app-frame";
...
export { QUOTE_TO_PAID_DURATION } from "../durations";
...
const invoiceAt = (frame: number) => progress(frame, TO_INVOICE, 18);
const isInvoiceAt = (frame: number) => invoiceAt(frame) > 0.5;
const statusAt = (frame: number) =>
	frame < SEND_CLICK + 8 ? "draft" : frame < APPROVED ? "sent" : "approved";
const clickAt = (frame: number) =>
	interpolate(frame, [SEND_CLICK - 4, SEND_CLICK, SEND_CLICK + 10], [0, 1, 0], {
		extrapolateLeft: "clamp",
		extrapolateRight: "clamp",
	});

// The send button is the last header control, so it sits flush against the
// header's right padding. Fixing its width keeps the cursor target exact
// instead of guessing at the rendered text run.
const SEND_SIZE = 22;
...
const QuoteToPaidHeader: React.FC = () => {
	const frame = useCurrentFrame();
	const isInvoice = isInvoiceAt(frame);
	return (
		<>
			<StatusBadge status={isInvoice ? (frame >= PAID ? "paid" : "sent") : statusAt(frame)} />
			{!isInvoice ? (
				<PrimaryButton
					pressed={clickAt(frame)}
					size={SEND_SIZE}
...
/** Cursor lives at composition scale, on top of the shell — not in the canvas. */
const QuoteToPaidCursor: React.FC = () => {
	const frame = useCurrentFrame();
	if (frame <= 40 || frame >= 160) return null;
	return (
		<Cursor
			x={interpolate(frame, [55, 85, 110, 140], [1240, SEND_TARGET.x, SEND_TARGET.x, 1400], {
				extrapolateLeft: "clamp",
				extrapolateRight: "clamp",
			})}
			y={interpolate(frame, [55, 85, 110, 140], [760, SEND_TARGET.y, SEND_TARGET.y, 700], {
				extrapolateLeft: "clamp",
				extrapolateRight: "clamp",
			})}
			click={clickAt(frame)}
			opacity={interpolate(frame, [150, 160], [1, 0], {
				extrapolateLeft: "clamp",
				extrapolateRight: "clamp",
			})}
		/>
...
export const QUOTE_TO_PAID_SHELL: SceneShell = {
	active: (frame) => (isInvoiceAt(frame) ? "Invoices" : "Quotes"),
	title: (frame) => (isInvoiceAt(frame) ? "Invoice #0847" : "Quote #1042"),
	// Quotes and Invoices are adjacent nav rows, so the reel's pill can slide the
	// gap in step with the flip instead of teleporting.
	activeIndex: (frame) =>
		interpolate(invoiceAt(frame), [0.35, 0.65], [NAV_INDEX.Quotes, NAV_INDEX.Invoices], {
			extrapolateLeft: "clamp",
			extrapolateRight: "clamp",
		}),
	HeaderAction: QuoteToPaidHeader,
	Overlay: QuoteToPaidCursor,
};

/** Draft quote → sent → client-approved → invoice → paid. */
export const QuoteToPaidContent: React.FC = () => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
	const t = useTheme();

	const invoicePhase = invoiceAt(frame);
					...
					>
						...
						{frame >= APPROVED ? (
							<StatusBadge
								...
								style={{
									marginLeft: "auto",
									scale: String(0.92 + pop(frame, fps, APPROVED) * 0.08),
									opacity: progress(frame, APPROVED, 8),
								}}
							/>
						) : null}
					</div>
...
export const QuoteToPaid: React.FC = () => (
	<AbsoluteFill>
		<SceneFrame shell={QUOTE_TO_PAID_SHELL}>
			<QuoteToPaidContent />
		</SceneFrame>
	</AbsoluteFill>
);

export default themed(QuoteToPaid);
