...
/**
 * Client mirror of lib/editLocks.ts. Keep both lists in step — the server is
 * authoritative (mutations throw INVOICE_LOCKED), this just keeps the grid from
 * offering edits that would bounce.
 */
const LOCKED_STATUSES: ReadonlyArray<Doc<"invoices">["status"]> = [
	"paid",
	"cancelled",
];

const SETTLED_PAYMENT_STATUSES: ReadonlyArray<Doc<"payments">["status"]> = [
	"paid",
	"refunded",
];

const LOCKED_REASON: Record<string, string> = {
	paid: "Paid invoices are locked. Line items can't change after payment.",
	cancelled:
		"Cancelled invoices are locked. Line items can't change once the invoice is voided.",
};

const SETTLED_PAYMENT_REASON =
	"A payment has been recorded, so line items and pricing are locked.";

const SETTLE_MS = 700;

/**
 * Mirrors validateInvoiceLineItemFields in the backend — these values are
...
/** Grid `rate`/`amount` map to invoice `unitPrice`/`total`. */
function toMutationFields(patch: GridLineItemPatch) {
	const { rate, ...rest } = patch;
	return rate === undefined ? rest : { ...rest, unitPrice: rate };
}

export interface UseInvoiceLineItemsControllerArgs {
	invoice: Doc<"invoices">;
	invoiceId: Id<"invoices">;
	lineItems: Doc<"invoiceLineItems">[] | undefined;
	/** Payment schedule rows for this invoice — a settled row locks the grid. */
	payments: Doc<"payments">[] | undefined;
	/** Internal cost/margin columns. Defaults to on. */
	showCostMargin?: boolean;
}

export function useInvoiceLineItemsController({
	invoice,
	invoiceId,
	lineItems,
	payments,
	showCostMargin = true,
}: UseInvoiceLineItemsControllerArgs): LineItemGridController {
	...
	const duplicateLineItem = useMutation(api.invoiceLineItems.duplicate);
	const reorderLineItems = useMutation(api.invoiceLineItems.reorder);
	const bulkCreateLineItems = useMutation(api.invoiceLineItems.bulkCreate);
	...
	const hasSettledPayment = useMemo(
		() =>
			(payments ?? []).some(
				(row) =>
					SETTLED_PAYMENT_STATUSES.includes(row.status) ||
					row.disputed === true
			),
		[payments]
	);

	const statusLocked = LOCKED_STATUSES.includes(invoice.status);
	const locked = statusLocked || hasSettledPayment;
	// Status first, then the settled-payment fallback — matches the server's
	// check order in assertInvoiceContentEditable.
	const lockedReason = statusLocked
		? LOCKED_REASON[invoice.status]
		: hasSettledPayment
			? SETTLED_PAYMENT_REASON
			: undefined;
	...
	const patchItem = useCallback(
		async (id: string, patch: GridLineItemPatch): Promise<boolean> => {
			...
			const result = await run("Couldn't save the line", () =>
				updateLineItem({
					id: id as Id<"invoiceLineItems">,
					...toMutationFields(patch),
				})
			);
			return result !== null;
		},
	...
	}, [createLineItem, invoiceId, itemCount, run]);

	const removeItems = useCallback(
		async (ids: string[]) => {
			await run("Couldn't remove the lines", async () => {
				for (const id of ids) {
					await removeLineItem({ id: id as Id<"invoiceLineItems"> });
				}
			});
		},
		[removeLineItem, run]
	...
	const duplicateItems = useCallback(
		async (ids: string[]) => {
			await run("Couldn't duplicate the lines", async () => {
				for (const id of ids) {
					await duplicateLineItem({ id: id as Id<"invoiceLineItems"> });
				}
			});
		},
	...
	const reorderItems = useCallback(
		async (orderedIds: string[]) => {
			...
			await run("Couldn't reorder the lines", () =>
				reorderLineItems({
					invoiceId,
					lineItemIds: orderedIds as Id<"invoiceLineItems">[],
				})
			);
		...
		[invoiceId, reorderLineItems, run]
	);
	...
	const bulkAddRows = useCallback(
		...
		[bulkCreateLineItems, invoiceId, itemCount, run, toast]
	);
