              ...
              <CommandGroup heading="Support">
                {filteredSupport.map((action) => (
                  <CommandItem
                    ...
                    onSelect={() => runSelect(() => openSupport?.(action.key))}
                  >
                    <action.icon />
                    <span>{action.label}</span>
                  </CommandItem>
...
export function CommandPaletteTrigger() {
  ...
  return (
    <button
      ...
      className="workspace-global-search flex h-9 w-full min-w-0 items-center gap-2 rounded-md border border-sidebar-border px-3 text-sm text-foreground transition-colors hover:bg-sidebar-accent focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring"
    >
      ...
      <Kbd className="ml-auto hidden border-sidebar-border bg-sidebar-accent text-foreground sm:inline-flex">
        &#8984;K
      </Kbd>
    </button>
  );
