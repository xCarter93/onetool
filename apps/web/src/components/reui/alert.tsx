...
// Vendored from the ReUI registry (@reui/alert, radix-nova style) minus the
// `invert` variant — the app's theme has no --invert tokens. The info/success/
// warning variants ride on the app's existing tokens in globals.css.
	...
	],
	{
		variants: {
			variant: {
				...
				// Deliberate accent (the app's semantic tokens are blue). Reproduces
				// ReUI's violet "info" look for the unpublished-changes banner.
				violet:
					"border-violet-500/30 bg-violet-500/5 [&>svg]:text-violet-600 dark:border-violet-400/30 dark:bg-violet-400/10 dark:[&>svg]:text-violet-400",
			},
		},
		defaultVariants: {
			variant: "default",
