...
function createColumns(
  ...
  users: { _id: Id<"users">; name?: string; email: string }[] | undefined,
      ...
      cell: ({ row }) => {
        ...
        return (
          <Badge variant="outline" className="gap-1.5 font-normal text-xs">
            ...
            <span className="truncate max-w-[120px]">{client.companyName}</span>
          </Badge>
        );
      },
    },
...
function TasksPageContent() {
  ...
  // A whitespace-only search or a filter row with no values narrows nothing —
  // the list is genuinely empty, so it must not read as "no matches".
  const isNarrowed =
    searchQuery.trim() !== "" || filters.some((f) => f.values.length > 0);
  ...
  const overdueTasks =
    allTasks?.filter((t) => t.date < todayStart && !isTerminalStatus(t.status))
      .length || 0;
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="bg-card rounded-lg border">
          <EmptyState
            size="md"
            illustration={isNarrowed ? "no-filter-match" : "tasks-none"}
