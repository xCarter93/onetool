...
vi.mock("@/components/ui/radio-group", () => ({
	RadioGroup: ({
		...
		<div data-value={value} onClick={() => onValueChange("per_visit")}>
			{children}
		</div>
	),
	RadioGroupItem: ({ value }: { value: string }) => (
		<span data-testid={`billing-${value}`} />
	),
}));
	...
	RecurringPaymentRuleEditor: ({
		...
		<button
			...
			onClick={() =>
				onChange({
					type: "percentage",
					installments: [{ percentage: 75, dayOffset: 5 }],
				})
			}
		>
			Payment: {value.type}
...
const savedTerms = {
	...
	schedule: {
		rule: { frequency: "monthly" as const, interval: 3 },
...
function Setup({ terms = savedTerms }: { terms?: typeof savedTerms }) {
	return (
		<RecurringAgreementSetupDialog
			...
			quoteTitle="Spring service"
			seriesRevision={4}
			seriesSetup={seriesSetup}
			savedTerms={terms as never}
		>
			{(open) => <button onClick={open}>Open setup</button>}
		</RecurringAgreementSetupDialog>
	);
}
	...
	it("keeps in-progress edits when saved terms refresh", () => {
		...
		view.rerender(
			<Setup
				terms={{
					...savedTerms,
					scope: { title: "Server refresh", description: "Other" },
				}}
			/>,
		);
