...
vi.mock("@onetool/backend/convex/_generated/api", () => ({
	api: { payments: { configurePaymentsWithScope: "configure" } },
}));
...
describe("PaymentsConfigurationModal recurring scope", () => {
	...
	it("saves the reusable rule with the expected agreement revision", async () => {
		configure.mockResolvedValue({
			...
			futureProposal: { quoteIds: ["quote-2"] },
		});
		render(
			<PaymentsConfigurationModal
				isOpen
		...
		expect(
			await screen.findByRole("button", { name: "Review agreement revision" }),
		).toHaveAttribute("href", "/quotes/quote-2");
	});
});
