...
interface CommandHeroProps {
	...
	compact?: boolean;
	/**
	 * iPad pane: the rail already owns the org chip, the bell and the avatar, and
	 * the status bar spans the light rail too — so the hero drops its chrome and
	 * leaves the glyphs dark.
	 */
	hideChrome?: boolean;
...
export function CommandHero({
	...
	// Today has no AppHeader on iPhone, so the hero carries the global search jump
	// the other tab roots get from app-header.tsx. One element, two homes: the org
	// bar on phone, the greeting line in a pane.
	const searchButton = (
		<InkIconButton
	...
	return (
		...
		>
			...
			{isFocused && !hideChrome ? <StatusBar style="light" /> : null}
			{size.width > 0 && (
				<Svg
					width={size.width}
					height={size.height}
			...
			{/* Org bar. With the rail owning identity, the pane has nothing left to
			    put here, so the search jump rides the greeting line instead. */}
			{hideChrome ? null : (
			...
			{hideChrome ? (
				<View style={styles.greetingRow}>
					<Text
						style={[
							styles.greeting,
							compact && styles.greetingCompact,
							styles.greetingFill,
						]}
					>
						{greeting}
...
const styles = StyleSheet.create({
	...
	// The row owns the top margin, so the greeting sits centred against the glyph.
	greetingFill: {
		flex: 1,
		marginTop: 0,
	},
