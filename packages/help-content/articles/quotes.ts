			...
			},
			{
				...
				blocks: [
					{ type: "steps", items: [
						...
						"Choose **Use for this series** from the quote’s actions, then **Copy drafts only**. Review the counts, then choose **Copy drafts**.",
					...
					{ type: "paragraph", text: "Choose **Use for this series** on a draft quote in an active series, then **Set up a recurring agreement**, the recommended option. It saves the proposed service scope, cadence, billing rhythm and payment schedule. Reopening setup restores those saved terms so you can continue the same proposal. Then click **Generate PDF** on the quote to create the approval version. The quote header shows the agreement steps **Set up**, **PDF**, **Sent** and **Approved**, and the series page shows the same state. The setup dialog shows the current series schedule; open **Change schedule** only if the agreement should change the cadence, and the series page then shows the change before the client sees it. If an agreement quote was marked approved by hand before this rule existed, the series page shows **Not activated**; withdraw the proposal, then send it to the client again. If an unsent agreement PDF already exists, OneTool asks before replacing it; if the client has already received a proposal, withdraw it from the series page first. While a series has an agreement, **Copy drafts only** is unavailable and future visits change through **Revise agreement** on the series page." },
		...
		sections: [
			...
			},
			{
				...
				blocks: [
					...
					{
						type: "note",
						text: "Recurring agreement quotes cannot be marked approved by hand. Their approval has to come from the client, through the portal, an in-person signature on the mobile app, or an e-signature request, because that approval covers every future visit. Use **Send to Client** as with any quote: the portal template or a custom email lets the client approve in the portal, and **Send for e-signature** sends the locked agreement PDF for signature. Mark Approved is not offered on agreement quotes.",
					},
				],
			},
			{
