...
export function formatRecurringSchedule(rule: ProjectRecurrenceRule): string {
	...
	const cadence = rule.interval === 1 ? rule.frequency : `Every ${rule.interval} ${unit}s`;
	...
	const season = rule.months?.length
		...
		: "";
	const ending = rule.end?.kind === "until"
...
const describeDueOffset = (days: number) =>
	days === 0 ? "when issued" : `${days} day${days === 1 ? "" : "s"} after issue`;

export function formatRecurringPaymentRule(rule: AgreementPaymentRule): string {
	if (rule.type === "percentage")
		return rule.installments
			.map((item) => `${item.percentage}% due ${describeDueOffset(item.dayOffset)}`)
			.join("; ");
	return [
		...rule.installments.map(
			(item) => `${formatCurrency(item.amount)} due ${describeDueOffset(item.dayOffset)}`
		),
