...
async function requireScheduleAccess(ctx: UserQueryCtx, modify: boolean) {
	...
	if (
		...
		throw new ConvexError(
			"Organization-wide project, quote and invoice access is required for shared payment schedules",
		);
}
...
export const getPending = userQuery({
	...
	returns: v.union(
		v.null(),
		v.object({
			versionId: v.id("clientMonthlyPaymentScheduleVersions"),
			status: v.union(v.literal("pending_approval"), v.literal("scheduled")),
	...
	handler: async (ctx, args) => {
		...
		const pointer = await ctx.db
			...
			.withIndex("by_org_client", (q) =>
				q.eq("orgId", ctx.orgId).eq("clientId", args.clientId),
			)
			.unique();
		if (!pointer?.pendingVersionId) return null;
		const version = await ctx.orgEntity(
			"clientMonthlyPaymentScheduleVersions",
			pointer.pendingVersionId,
		);
		if (version.status !== "pending_approval" && version.status !== "scheduled")
			return null;
		...
		for (const revisionId of version.agreementRevisionIds) {
			...
			if (series.activeAgreementRevisionId === revisionId)
				partiallyActivated = true;
		}
		const org = await ctx.db.get(ctx.orgId);
		if (!org) throw new ConvexError("Organization is unavailable");
		const alreadyEffective = Boolean(
			version.effectiveMonth &&
			version.effectiveMonth <=
				dateKeyFromTimestamp(Date.now(), org.timezone ?? "UTC").slice(0, 7),
		);
		return {
			versionId: version._id,
			status: version.status,
			effectiveMonth: version.effectiveMonth,
			approvedCount: version.approvedSeriesIds.length,
			requiredCount: version.seriesIds.length,
			canCancel: !partiallyActivated && !alreadyEffective,
			cancellationReason: partiallyActivated
				? "The proposal has started activating"
				: alreadyEffective
					? "The payment schedule is already effective"
					: undefined,
		};
	},
});
...
export const cancelPending = userMutation({
	args: {
		...
		expectedVersionId: v.id("clientMonthlyPaymentScheduleVersions"),
	},
	returns: v.null(),
	handler: async (ctx, args) => {
		await requireScheduleAccess(ctx, true);
		await ctx.orgEntity("clients", args.clientId);
		await cancelMonthlyPaymentProposal(ctx, {
			orgId: ctx.orgId,
...
export const activateClient = internalMutation({
	args: { orgId: v.id("organizations"), clientId: v.id("clients") },
	returns: v.null(),
	handler: async (ctx, args) => {
...
export const sweep = internalMutation({
	...
	handler: async (ctx, args) => {
		const versions = await ctx.db
			...
			.withIndex("by_status_month", (q) => q.eq("status", "scheduled"))
		...
		for (const version of versions.page)
			await ctx.scheduler.runAfter(
				...
				{ orgId: version.orgId, clientId: version.clientId },
			);
		if (!versions.isDone)
			await ctx.scheduler.runAfter(
				0,
				internal.recurringPaymentSchedules.sweep,
				{ cursor: versions.continueCursor },
			);
		return null;
	},
});
