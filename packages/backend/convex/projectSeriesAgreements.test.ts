...
describe("recurring agreements", () => {
	...
	it("captures a pending immutable revision without publishing its quote template", async () => {
		...
		expect(prepared.terms.scope).toEqual({
			title: "Approved cleaning scope",
			description: "Kitchen and bath",
		});
		expect(prepared.terms.schedule.rule).toEqual({
			frequency: "weekly",
			interval: 2,
			end: { kind: "count", count: 8 },
		});
		...
		).toEqual(prepared.terms);
		expect(quote!.recurringAgreementTerms?.paymentRule).toEqual({
			type: "percentage",
			installments: [{ percentage: 100, dayOffset: 30 }],
		});
	...
	it("discards an unsent revision while preserving its history and source references", async () => {
		...
		expect(revision).toMatchObject({
			...
			quoteVersionId: expect.any(String),
	...
	it("does not expose or discard another organization's agreement", async () => {
		...
		const outsider = await t.run(async (ctx) =>
			createTestOrg(ctx, {
				clerkOrgId: "org_outside",
				clerkUserId: "user_outside",
			}),
		);
		const otherUser = t.withIdentity(
			createTestIdentity(outsider.clerkUserId, outsider.clerkOrgId),
		);
		await expect(
			otherUser.query(api.projectSeriesAgreements.getSeriesAgreement, {
				seriesId: f.seriesId,
			}),
		).rejects.toThrow();
		await expect(
			otherUser.mutation(api.projectSeriesAgreements.discardPending, {
				seriesId: f.seriesId,
				expectedRevisionId: prepared.revisionId,
			}),
		).rejects.toThrow();
	...
	it("marks a reopened inherited visit as an override and removes inherited approval provenance", async () => {
		...
		await t.run(async (ctx) => {
			...
			await db.patch(f.quoteId, {
				status: "approved",
				approvedAt: 10,
				recurringInheritedAt: 10,
			});
