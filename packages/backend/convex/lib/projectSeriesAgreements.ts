...
import {
	applyApprovedAgreementToExistingProjects,
...
import {
	approveMonthlyPaymentRevision,
	assertMonthlyAgreementTerms,
} from "./recurringPaymentChanges";
...
async function activeAgreementSetup(
	ctx: MutationCtx,
	activeRevision: Doc<"projectSeriesAgreementRevisions">,
) {
	if (activeRevision.status !== "approved" || !activeRevision.terms)
		throw new ConvexError("Only an approved agreement can be revised");
	const [series, template, version] = await Promise.all([
		ctx.db.get(activeRevision.seriesId),
		ctx.db.get(activeRevision.templateId),
		ctx.db.get(activeRevision.quoteVersionId),
	]);
	if (
		!series ||
		!template ||
		!version ||
		series.orgId !== activeRevision.orgId ||
		series.activeAgreementRevisionId !== activeRevision._id
	)
		throw new ConvexError("Active recurring agreement setup is missing");
	return { series, template, version };
}

export async function createAgreementRevisionQuote(
	ctx: MutationCtx,
	activeRevision: Doc<"projectSeriesAgreementRevisions">,
	createdByUserId: Id<"users">,
) {
	const { series, template, version } = await activeAgreementSetup(
		ctx,
		activeRevision,
	);
	...
	const quoteId = await ctx.db.insert("quotes", {
		...
		createdByUserId,
		recurringAgreementSourceQuoteId: series.agreementQuoteId,
	});
	await writeRecurringQuoteLines(ctx, quoteId, version);
	return quoteId;
}

export async function createAgreementRevisionDraft(
	ctx: MutationCtx,
	activeRevision: Doc<"projectSeriesAgreementRevisions">,
	createdByUserId: Id<"users">,
	paymentRule: RecurringPaymentRule,
	monthlyPaymentScheduleVersionId?: Id<"clientMonthlyPaymentScheduleVersions">,
) {
	...
	if (series.pendingAgreementRevisionId)
		throw new ConvexError(
			"Finish the pending agreement revision before changing future payments",
		);
	const quoteId = await createAgreementRevisionQuote(
		ctx,
		activeRevision,
		createdByUserId,
	);
	...
	const quoteVersionId = await ctx.db.insert("projectSeriesQuoteVersions", {
		orgId: series.orgId,
		seriesId: series._id,
		sourceQuoteId: series.agreementQuoteId ?? version.sourceQuoteId,
	...
	const terms: RecurringAgreementTerms = {
		...
		paymentRule,
		monthlyPaymentScheduleVersionId,
		paymentChangeActivation: monthlyPaymentScheduleVersionId
			? "next_full_month_after_all_approvals"
			: undefined,
	};
...
async function visitInvoiced(
	...
	return Boolean(
		...
		(await ctx.db
			.query("invoiceGroups")
			.withIndex("by_source_quote", (q) => q.eq("sourceQuoteId", quoteId))
			.first()) ||
		(await ctx.db
			.query("invoiceGroups")
			.withIndex("by_source_project", (q) => q.eq("sourceProjectId", projectId))
			.first()),
	);
}

// A visit can take the active agreement's pricing back when it is a protected
// override or a draft copy left behind by an earlier saved version.
export async function restorableAgreementPricing(
	ctx: Pick<QueryCtx, "db">,
	quote: Doc<"quotes">,
	project: Doc<"projects">,
	series: Doc<"projectSeries">,
) {
	if (series.agreementReviewRequired || !series.activeAgreementRevisionId)
		return null;
	const revision = await ctx.db.get(series.activeAgreementRevisionId);
	if (
		!revision ||
	...
	const ledger = await ctx.db
		...
		.unique();
	const staleLinkedDraft =
		quote.status === "draft" &&
		!quote.recurringAgreementSourceQuoteId &&
		ledger !== null &&
		ledger.templateId === revision.templateId &&
		ledger.versionId !== revision.quoteVersionId;
	if (!quote.recurringQuoteOverride && !staleLinkedDraft) return null;
	if (await visitInvoiced(ctx, quote._id, project._id)) return null;
	return { revision, ledger };
}

async function startedOrBilled(
	ctx: Pick<QueryCtx, "db">,
	project: Doc<"projects">,
) {
	if (
	...
	return Boolean(
		...
		(await ctx.db
			...
			.withIndex("by_source_project", (q) =>
				q.eq("sourceProjectId", project._id),
			)
			.first()),
	);
}

async function applyApprovedSeriesTerms(
	ctx: MutationCtx,
	series: Doc<"projectSeries">,
	terms: RecurringAgreementTerms,
	revision: number,
) {
	const ruleError = validateRecurrenceRule(
		terms.schedule.rule,
		terms.schedule.anchorDateKey,
	);
	...
	const visits = await ctx.db
		...
		.withIndex("by_series_start", (q) =>
			q.eq("recurringSeriesId", series._id).gte("startDate", start),
		)
		.take(201);
	if (visits.length > 200)
		throw new ConvexError("Agreement change affects more than 200 visits");
	const cadenceChanged =
		JSON.stringify(series.rule) !== JSON.stringify(terms.schedule.rule);
	const selected = new Set(
		cadenceChanged
			? listRecurrenceDates({
					rule: terms.schedule.rule,
	...
	for (const visit of visits) {
		...
		if (!overrides.includes("description"))
			updates.description = terms.scope.description;
		if (cadenceChanged) {
			const date = visit.recurringNominalDate ?? "";
			const restoring =
				visit.recurringSkipReason === "schedule-change" && selected.has(date);
			const removing =
			...
			if (restoring || removing) {
				...
				if (
					...
					updates.recurringSkipReason = restoring
						? undefined
						: "schedule-change";
				}
			}
	...
	await ctx.db.patch(series._id, {
		title: terms.scope.title,
		description: terms.scope.description,
		rule: terms.schedule.rule,
...
function comparableQuoteVersion(
	value:
		| Awaited<ReturnType<typeof snapshotQuoteVersion>>
...
export async function assertAgreementSourceCurrent(
	ctx: Pick<QueryCtx, "db">,
	quote: Doc<"quotes">,
	...
	if (
		!version ||
		version.capturedFromQuoteId !== quote._id ||
		!revision.terms ||
		quote.recurringAgreementTerms?.revisionId !== revision._id
	)
		throw new ConvexError(
			"Prepare the updated recurring agreement before generating it",
		);
	const current = await snapshotQuoteVersion(ctx, quote);
	if (
		JSON.stringify(comparableQuoteVersion(current)) !==
		JSON.stringify(comparableQuoteVersion(version))
	)
		throw new ConvexError(
			"Prepare the updated recurring agreement before generating it",
		);
...
export async function activateAgreementApproval(
	...
	if (
		!revision ||
		revision.sourceQuoteId !== quoteId ||
	...
	const unsafeVendorDocument =
		evidence.channel === "boldsign" &&
		(document?.recurringAgreementLocked !== true ||
			document.recurringAgreementEditedAt !== undefined);
	if (
		revision.status !== "pending" ||
		!document ||
		unsafeVendorDocument ||
		evidence.contentBinding !== "rendered_document" ||
		evidence.snapshotSource !== "server" ||
		evidence.documentId !== revision.approvalDocumentId ||
		evidence.contentSnapshot?.approvalCycle !== revision.approvalCycle ||
		evidence.contentSnapshot?.recurringAgreementTerms?.revisionId !==
			revision._id
	)
		throw new ConvexError(
			"This approval is not bound to the pending recurring agreement version",
		);
	...
	if (
		...
		series.agreementQuoteId !==
			(quote.recurringAgreementSourceQuoteId ?? quoteId)
	)
	...
	if (
		!template ||
		template.seriesId !== series._id ||
		template.sourceQuoteId !== series.agreementQuoteId
	)
		throw new ConvexError("Recurring agreement quote setup is missing");
	const quoteVersion = await ctx.db.get(revision.quoteVersionId);
	if (!quoteVersion)
		throw new ConvexError("Recurring agreement quote version is missing");
	...
	if (await approveMonthlyPaymentRevision(ctx, revision, evidence))
		return revision._id;
	...
	await applyApprovedAgreementRevision(ctx, {
		...revision,
		status: "approved",
		decisionEvidenceId: evidence._id,
		approvedAt: now,
...
export async function applyApprovedAgreementRevision(
	ctx: MutationCtx,
	revision: Doc<"projectSeriesAgreementRevisions">,
) {
	if (
		...
		throw new ConvexError(
			"Approved recurring agreement provenance is incomplete",
		);
	const [series, template, quoteVersion, quote] = await Promise.all([
		ctx.db.get(revision.seriesId),
		ctx.db.get(revision.templateId),
		ctx.db.get(revision.quoteVersionId),
		ctx.db.get(revision.sourceQuoteId),
	]);
	if (
		!series ||
		!template ||
	...
	await ctx.db.patch(template._id, {
		versionId: revision.quoteVersionId,
	...
	if (
		...
		series.activeAgreementRevisionId !== revision._id
	) {
...
export function vendorRequestLive(document: Doc<"documents">) {
	return (
		Boolean(document.recurringSignatureSendState) ||
		(document.boldsign !== undefined &&
			!["Revoked", "Declined", "Expired", "Completed"].includes(
				document.boldsign.status,
			))
	);
}

export async function bindAgreementApprovalDocument(
	ctx: MutationCtx,
	quoteId: Id<"quotes">,
	...
	const bound = revision.approvalDocumentId
		...
		: null;
	if (bound && bound._id !== documentId && vendorRequestLive(bound))
		throw new ConvexError(
			"Withdraw the delivered agreement signature request before generating a new PDF",
		);
	...
	if (
		...
		document.documentId !== quoteId ||
		document.quoteSnapshotSource !== "server" ||
		(!document.quoteContentSnapshotId && !document.quoteContentSnapshot)
	)
		throw new ConvexError(
			"Recurring agreements require a controlled server-rendered document",
		);
	await ctx.db.patch(revision._id, {
...
export async function latestAgreementRevision(
	ctx: Pick<QueryCtx, "db">,
	seriesId: Id<"projectSeries">,
) {
	return ctx.db
		.query("projectSeriesAgreementRevisions")
		.withIndex("by_series_revision", (q) => q.eq("seriesId", seriesId))
		.order("desc")
		.first();
}
...
export async function discardPendingAgreementRevision(
	...
	if (
		...
		quote.recurringAgreementRevisionId !== revision._id
	)
	...
	if (
		...
		throw new ConvexError(
			"Revoke the signature request before withdrawing this agreement",
		);
	const now = Date.now();
	const resetDelivery =
		quote.status === "sent" ||
	...
	if (resetDelivery) {
		await emitStatusChangeEvent(
			...
			quote.status,
			"draft",
			"projectSeriesAgreements.withdraw",
		);
	}
	await ctx.db.patch(series._id, {
		pendingAgreementRevisionId: undefined,
		agreementQuoteId: series.activeAgreementRevisionId
			? series.agreementQuoteId
			: undefined,
		revision: (series.revision ?? 0) + 1,
	});
