...
export function recurringPaymentRuleKey(rule: RecurringPaymentRule): string {
  return JSON.stringify(rule.type === "percentage"
    ? { type: rule.type, installments: rule.installments.map(({ percentage, dayOffset }) => ({ percentage, dayOffset })) }
    : { type: rule.type, installments: rule.installments.map(({ amount, dayOffset }) => ({ amount: roundCents(amount), dayOffset })), balance: { dayOffset: rule.balance.dayOffset } });
}
...
function dueDate(
  ...
  try {
    ...
    result = Date.parse(`${addCalendarDays(issueDate, dayOffset)}T00:00:00.000Z`);
  } catch (error) {
    fail(
      "arithmetic_overflow",
  ...
  if (rule.type === "percentage") {
    ...
    const installments = rule.installments.map((item, index) => {
      ...
      const paymentCents = isFinal
        ...
        : checkedCents(
            roundCents(canonicalTotal * (item.percentage / 100)),
            "zero_installment",
          );
