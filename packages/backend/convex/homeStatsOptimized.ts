...
/**
 * Aggregate-based home stats computation shared by the web and mobile home
 * dashboards. Uses O(log n) aggregate reads instead of full-table `.collect()`
 * scans so a single query never trips Convex's document-read limit on large
 * orgs. Callers are responsible for permission checks before invoking this.
 */
export async function computeHomeStats(
	...
	const projectsChange = completedProjectsThisMonth - completedProjectsLastMonth;
	...
	if (monthlyTarget === 0) {
		...
		currentPercentage = Math.round((currentRevenueValue / monthlyTarget) * 100);
