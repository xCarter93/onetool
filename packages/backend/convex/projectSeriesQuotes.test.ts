...
describe("recurring project quote copy-forward", () => {
	...
	async function fixture(count = 5, interval = 1) {
		...
		const quoteId = await user.mutation(api.quotes.create, {
			...
			subtotal: 0,
			total: 0,
			terms: "Access required before 9 AM",
			clientMessage: "Weekly service",
	...
	const quotes = (projectId: Id<"projects">) =>
		t.run((ctx) =>
			ctx.db
				.query("quotes")
	...
	async function copy(f: Fixture, quoteId = f.quoteId) {
		...
		const result = await f.user.mutation(api.projectSeriesQuotes.copy, {
			quoteId,
	...
	const setup = (f: Fixture) =>
		f.user.query(api.projectSeriesQuotes.getSetup, { projectId: f.projectId });
	...
	it("creates independent drafts with exact amounts, fresh numbers and fresh line items", async () => {
		...
		for (const visit of (await visits(f)).slice(1)) {
			...
			const copiedLines = (await lines(copied._id)).sort(
			...
			expect(
				copiedLines.every(
					(line) => !sourceLines.some((source) => source._id === line._id),
				),
			).toBe(true);
		}
	...
	it("updates only the same source's draft copy while preserving manual and other-source quotes", async () => {
		...
		const manualId = await f.user.mutation(api.quotes.create, {
			clientId: f.clientId,
		...
		const secondId = await f.user.mutation(api.quotes.create, {
			clientId: f.clientId,
			projectId: f.projectId,
			title: "Separate offer",
			status: "draft",
			subtotal: 0,
			total: 0,
		});
		...
		await f.user.mutation(api.quotes.update, {
			id: first._id,
			title: "Ordinary draft edit",
		});
		await f.user.mutation(api.quoteLineItems.update, {
			id: (await lines(first._id))[0]._id,
			quantity: 10,
		});
		await f.user.mutation(api.quotes.update, {
			id: f.quoteId,
		...
		expect(after.find((quote) => quote._id === first._id)).toMatchObject({
			title: "Updated scope",
			quoteNumber: first.quoteNumber,
			subtotal: 89.96,
		});
		expect(
			(await lines(first._id)).find((line) => line.sortOrder === 0)?.quantity,
		).toBe(4);
		expect(after.find((quote) => quote._id === manualId)?.title).toBe(
			"Manual extra",
		);
		expect(
			after.filter((quote) => quote.title === "Separate offer"),
		).toHaveLength(1);
	});
	...
	it("retains the last published source version for later generation after source edits and deletion", async () => {
		...
		const later = (await visits(f)).find(
			(visit) => visit.startDate === START + 112 * DAY,
		)!;
		expect((await quotes(later._id))[0]).toMatchObject({
			title: "Cleaning scope",
			subtotal: 69.97,
			total: 69.79,
			status: "draft",
		});
		expect((await setup(f))!.templates[0]).toMatchObject({
	...
	it("stops future generation while leaving all existing quotes and line items unchanged", async () => {
		...
		await f.user.mutation(api.projectSeriesQuotes.stop, {
			templateId: current.templates[0]._id,
	...
	it("rejects stale previews after source line edits, target edits, or quote status changes", async () => {
		const f = await fixture(3);
		let preview = await f.user.query(api.projectSeriesQuotes.previewCopy, {
			quoteId: f.quoteId,
		});
		await f.user.mutation(api.quoteLineItems.update, {
			id: f.firstLineId,
			description: "Changed at same timestamp",
		});
		await expect(
		...
		preview = await f.user.query(api.projectSeriesQuotes.previewCopy, {
			quoteId: f.quoteId,
		});
		await f.user.mutation(api.quoteLineItems.update, {
			id: (await lines(target._id))[0]._id,
			description: "Target draft change",
		});
		await expect(
			f.user.mutation(api.projectSeriesQuotes.copy, {
				quoteId: f.quoteId,
	...
	it("keeps sent or approved copies protected even after they are reopened as drafts", async () => {
		...
		const targets = await Promise.all(
			(await visits(f))
				.slice(1)
				.map(async (project) => (await quotes(project._id))[0]),
		);
		await f.user.mutation(api.quotes.update, {
			id: targets[0]._id,
			status: "sent",
		...
		await copy(f);
		expect((await quotes(targets[0].projectId!))[0].title).toBe(
			"Cleaning scope",
		);
		expect((await quotes(targets[1].projectId!))[0].title).toBe(
			"Cleaning scope",
		);
		expect((await quotes(targets[2].projectId!))[0].title).toBe(
			"Published change",
		);
	});
	...
	it("excludes suppressed, started, billed and past visits from propagation", async () => {
		...
		await f.user.mutation(api.tasks.create, {
			...
			title: "Work started",
			status: "in-progress",
			date: rows[2].startDate!,
		});
		await f.user.mutation(api.invoices.create, {
			clientId: f.clientId,
			projectId: rows[3]._id,
			invoiceNumber: "COPY-1",
			status: "draft",
			subtotal: 10,
			total: 10,
			issuedDate: NOW,
		...
		for (const row of rows.slice(1, 5))
			expect(await quotes(row._id)).toHaveLength(0);
	...
	it("never emits inherited approval events or creates approval audit records", async () => {
		...
		const events = await t.run(async (ctx) =>
			(await ctx.db.query("domainEvents").collect()).filter(
				(event) => event.eventSource === "projectSeriesQuotes.copy",
			),
		);
		expect(events).toHaveLength(2);
		expect(
			events.every((event) => event.eventType === "entity.record_created"),
		).toBe(true);
		await copy(f);
		const updates = await t.run(async (ctx) =>
			(await ctx.db.query("domainEvents").collect()).filter(
				(event) =>
					event.eventSource === "projectSeriesQuotes.copy" &&
		...
		expect(
			changed.every(
				(event) =>
					JSON.stringify(event.payload.metadata?.changedFields) ===
	...
	it("blocks ordinary draft-copy propagation once a recurring agreement is designated", async () => {
		...
		await t.run(async (ctx) => {
			await triggers
				.wrapDB(ctx)
				.db.patch(f.seriesId, { agreementQuoteId: f.quoteId });
		});
		expect((await setup(f))!.canCopy).toBe(false);
		await expect(
			f.user.query(api.projectSeriesQuotes.previewCopy, { quoteId: f.quoteId }),
	...
	it("requires stopping saved quote setup before moving the series to a different client", async () => {
		const f = await fixture(3);
		await copy(f);
		const nextClient = await t.run((ctx) =>
			createTestClient(ctx, f.orgId, { companyName: "Different customer" }),
		);
		await expect(
			f.user.mutation(api.projectSeries.updateFuture, {
				projectId: f.projectId,
				updates: { clientId: nextClient },
			}),
		).rejects.toThrow(/stop.*quote|quote.*stop/i);
		expect((await t.run((ctx) => ctx.db.get(f.seriesId)))!.clientId).toBe(
			f.clientId,
		);
		await f.user.mutation(api.projectSeries.updateFuture, {
			projectId: f.projectId,
			updates: { title: "Weekly window care" },
		});
		const current = (await setup(f))!;
		await f.user.mutation(api.projectSeriesQuotes.stop, {
			templateId: current.templates[0]._id,
