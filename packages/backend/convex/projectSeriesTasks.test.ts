...
describe("recurring project task copy-forward", () => {
	...
	async function fixture(count = 5, interval = 1) {
		...
		const org = await t.run(async (ctx) => {
			const setup = await createTestOrg(ctx, {
				clerkUserId: `copy_user_${n}`,
				clerkOrgId: `copy_org_${n}`,
			});
	...
	it("repeated copying updates untouched copies in place and leaves manual tasks alone", async () => {
		...
		const manual = await f.user.mutation(api.tasks.create, {
			clientId: f.clientId,
			projectId: future._id,
			title: "Manual extra",
			date: future.startDate!,
			status: "pending",
		});
		await f.user.mutation(api.tasks.update, {
			id: f.taskId,
			title: "Wash windows",
			description: "New saved scope",
		});
		await copy(f);
		await copy(f);
		const after = await tasks(future._id);
		expect(after).toHaveLength(2);
		expect(after.find((task) => task._id === first._id)).toMatchObject({
			title: "Wash windows",
			description: "New saved scope",
		});
		expect(after.find((task) => task._id === manual)?.title).toBe(
			"Manual extra",
		);
	});
	...
	it("preserves edits, started-then-reverted tasks, and permanently deleted copies", async () => {
		...
		const copies = await Promise.all(
			future.map(async (project) => (await tasks(project._id))[0]),
		);
		await f.user.mutation(api.tasks.update, {
			id: copies[0]._id,
			title: "One visit exception",
		});
		await f.user.mutation(api.tasks.update, {
			id: copies[1]._id,
			status: "in-progress",
	...
	it("uses the last explicitly copied snapshot for visits generated later, even after source deletion", async () => {
		...
		expect((await tasks(later._id))[0]).toMatchObject({
			title: "Clean windows",
			date: later.startDate! + DAY,
		});
		expect(await template(f)).toMatchObject({
			active: true,
	...
	it("rejects stale copy and removal previews after source or destination edits", async () => {
		const f = await fixture(3);
		let preview = await f.user.query(api.projectSeriesTasks.previewCopy, {
			taskId: f.taskId,
		});
		await f.user.mutation(api.tasks.update, {
			id: f.taskId,
			title: "Changed since preview",
		});
		await expect(
			f.user.mutation(api.projectSeriesTasks.copy, {
				taskId: f.taskId,
				expectedRevision: preview.revision,
			}),
		...
		await copy(f);
		const saved = await template(f);
		preview = await f.user.query(api.projectSeriesTasks.previewRemoval, {
			templateId: saved._id,
		});
		const target = (await visits(f))[1];
		await f.user.mutation(api.tasks.update, {
			id: (await tasks(target._id))[0]._id,
			title: "Keep mine",
		});
		await expect(
			f.user.mutation(api.projectSeriesTasks.remove, {
				templateId: saved._id,
				expectedRevision: preview.revision,
			}),
		).rejects.toThrow(/changed|preview/i);
	...
	it("keeps ongoing series manageable after more than 200 historical visits", async () => {
		...
		await f.user.query(api.projectSeriesTasks.previewCopy, {
			taskId: f.taskId,
		});
		await f.user.mutation(api.projectSeriesTasks.remove, {
			templateId: saved._id,
	...
	it("preserves task type and public-form trust provenance in copies and future generation", async () => {
		...
		expect((await tasks(next._id))[0]).toMatchObject({
			type: "internal",
			source: "public_form",
		});
		vi.setSystemTime(NOW + 60 * DAY);
	...
	it("emits task events for actual changes without retriggering automations on an unchanged recopy", async () => {
		...
		const copyEvents = () =>
			t.run(async (ctx) =>
				(await ctx.db.query("domainEvents").collect()).filter(
					(event) => event.eventSource === "projectSeriesTasks.copy",
				),
			);
		...
		await copy(f);
		const updates = (await copyEvents()).filter(
			(event) => event.eventType === "entity.record_updated",
		);
	...
	it("denies cross-tenant and assigned-only propagation", async () => {
		...
		await expect(
			foreign.user.query(api.projectSeriesTasks.previewCopy, {
				taskId: f.taskId,
			}),
		).rejects.toThrow();
		await expect(
			foreign.user.mutation(api.projectSeriesTasks.copy, {
				taskId: f.taskId,
				expectedRevision: 0,
			}),
		).rejects.toThrow();
		...
		await expect(
			asMember.query(api.projectSeriesTasks.previewCopy, { taskId: f.taskId }),
		).rejects.toThrow();
		await expect(
			asMember.mutation(api.projectSeriesTasks.copy, {
				taskId: f.taskId,
				expectedRevision: 0,
			}),
		).rejects.toThrow();
