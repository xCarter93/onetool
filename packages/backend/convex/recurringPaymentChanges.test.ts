...
describe("approved future payment arrangements", () => {
	...
	async function fixture(mode: "monthly" | "per_visit", count = 1) {
		const org = await t.run(async (ctx) => {
			...
			return { ...org, clientId: await createTestClient(ctx, org.orgId) };
		});
	...
	it("keeps the billed source intact and waits for per-visit agreement approval", async () => {
		...
		const result = await f.user.mutation(
			api.payments.configurePaymentsWithScope,
			{
		...
		expect(await t.run((ctx) => ctx.db.get(f.series[0].quoteId))).toEqual(
			originalQuote,
		);
		expect(
			(await t.run((ctx) => ctx.db.get(f.series[0].seriesId)))
				?.activeAgreementRevisionId,
		).toBe(f.series[0].revisionId);
		...
		await expect(
			f.user.mutation(api.invoices.createFromQuote, { quoteId: draftId }),
		).rejects.toThrow(/revision|visit/i);
	});
	...
	it("requires every monthly agreement then waits for the next full month", async () => {
		...
		let version = await t.run((ctx) =>
			ctx.db.get(proposal.monthlyScheduleVersionId!),
		);
		expect(version?.status).toBe("pending_approval");
		await t.mutation(internal.recurringPaymentSchedules.activateClient, {
			orgId: f.orgId,
			clientId: f.clientId,
		});
		for (const s of f.series)
			expect(
				(await t.run((ctx) => ctx.db.get(s.seriesId)))
					?.activeAgreementRevisionId,
			).toBe(s.revisionId);
		await approve(f.user, f.orgId, proposal.quoteIds[1]);
		version = await t.run((ctx) =>
			ctx.db.get(proposal.monthlyScheduleVersionId!),
		);
		...
		expect(
			(await t.run((ctx) => ctx.db.get(proposal.monthlyScheduleVersionId!)))
				?.status,
		).toBe("active");
		for (const s of f.series)
			expect(
